...
def create_invoice(request, pk):
    ...
    if request.method == 'POST':
        ...
        # ------------- send a simple email with link to invoice -------------
        try:
            invoice_url = request.build_absolute_uri(reverse('patient_invoice_view', args=[invoice.pk]))
            subject = f'Invoice {invoice.invoice_number} - Mahima Medicals'
            message = f'Hello {patient.user.get_full_name() or patient.user.username},\n\nAn invoice has been created for you. View it here:\n{invoice_url}\n\nThank you.'
            send_mail(subject, message, None, [patient.user.email], fail_silently=True)
        except Exception as e:
            # don't break invoice creation if email fails (fail silently or log)
...
# at top of views.py add needed imports
...
from django.core.mail import send_mail, EmailMessage # adjust import path if Payment model lives elsewhere
from hospital.models import Patient

# ------------------ patient invoices: list ------------------
  # patient login URL (use 'login' that your app uses)
from django.contrib.auth.decorators import login_required
from django.shortcuts import render
from pharmacy.models import Order, Sale
...
def patient_invoices(request):
    ...
    sales = Sale.objects.filter(sold_by=request.user).order_by('-sold_at')
...
# ------------------ patient view single invoice ------------------
@login_required(login_url='login')
def patient_invoice_view(request, pk):
    if not getattr(request.user, 'is_patient', False):
        return redirect('login')
    invoice = get_object_or_404(Payment, pk=pk, patient__user=request.user)
    return render(request, 'patient/invoice_view.html', {'invoice': invoice})
...
def download_invoice_pdf(request, pk):
    ...
    invoice = get_object_or_404(Payment, pk=pk, patient__user=request.user)
    template_path = 'patient/invoice_pdf.html'   # new template we will create
    ...
    return response
@login_required(login_url="login")
def patient_voice_call(request):
    return render(request, 'Extras/voice-call.html')
@login_required(login_url='login')
def patient_video_call(request):
    return render(request, 'Extras/video-call.html')
@login_required(login_url="login")
def patient_search_doctors(request):
    return render(request, 'Extras/search.html')

@login_required(login_url="login")
def patient_calendar(request):
    return render(request, 'Extras/calendar.html')

@login_required(login_url="login")
def patient_components(request):
    return render(request, 'Extras/components.html')

@login_required(login_url="login")
def patient_invoices(request):
    return render(request, 'Extras/invoices.html')

@login_required(login_url="login")
def patient_invoice_view(request):
    return render(request, 'Extras/invoice-view.html')
@login_required(login_url="login")
def patient_invoice_report(request):
    return render(request, 'Extras/invoice-report.html')
